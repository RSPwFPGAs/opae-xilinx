...
XSPI_Flasher::XSPI_Flasher( char *inMap )
{
...
bool XSPI_Flasher::finalTransfer(uint8_t *SendBufPtr, uint8_t *RecvBufPtr, int ByteCount)
{
   ...
   */
    ...
    if(TEST_MODE)
        std::cout << "1.Control/Status " << std::hex << ControlReg << "/" << StatusReg << std::dec << std::endl;
   ...
   */
    ...
    while (((StatusReg & XSP_SR_TX_FULL_MASK) == 0) && (RemainingBytes > 0)) {
	    ...
	    std::cout << "flashWrite error " << std::endl;
            return false;
        }
   ...
   */
    ...
    if(TEST_MODE)
        std::cout << "2.Control/Status " << std::hex << ControlReg << "/" << StatusReg << std::dec << std::endl;
 ...
 if(TEST_MODE)
        std::cout << "3.Control/Status " << std::hex << ControlReg << "/" << StatusReg << std::dec << std::endl;
    ...
    { /* Polled mode of operation */
        ...
        while(ByteCount > 0)
        {
            ...
            if(TEST_MODE)
                std::cout << "4.Control/Status " << std::hex << ControlReg << "/" << StatusReg << std::dec << std::endl;
...
int XSPI_Flasher::revertToMFG(void)
{
    ...
    //BITSTREAM_START_LOC = 0x001E0000;
    if(!writeBitstreamGuard(BITSTREAM_START_LOC)) {
        std::cout << "ERROR: Unable to set bitstream guard!" << std::endl;
        return -EINVAL;
    }
