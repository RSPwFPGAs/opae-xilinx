...
#define FME_FEATURE_PAC_HSSI_ETH    "fme_pac_hssi"
#define FME_FEATURE_S10_SDM_MB      "fme_s10_sdm_mb"
#define FME_FEATURE_AVMMI_BMC       "fme_avmmi_bmc"
#define FME_FEATURE_100G_EHIP       "fme_100g_ehip"
#define FME_FEATURE_MAX10_SPI       "fme_max10_spi"
#define FME_FEATURE_MAC_ROM_I2C     "fme_mac_rom_i2c"
...
#define PORT_FEATURE_IOPLL          "port_iopll"
...
struct feature_fme_emif_status {
	union {
		u64 csr;
		struct {
			u8 complete:2;		/* Initialization Complete */
			u8 rsvd1:6;		/* Reserved */
			u8 cal_failure:2;	/* Calibration Failure */
			u8 rsvd2:6;		/* Reserved */
			u8 active:2;		/* Clear State Machine Active */
			u64 rsvd3:46;		/* Reserved */
		};
	};
};
...
/* FME EMIF Control */
#define EMIF_CLEAR_MASK 0x3
struct feature_fme_emif_control {
	union {
		u64 csr;
		struct {
			u8 clear:2;	/* EMIF Clear State Machine Enable*/
			u64 rsvd:62;	/* Reserved */
		};
	};
};
...
enum fme_feature_id {
	...
	FME_FEATURE_ID_PAC_HSSI_ETH = 0xa,
	FME_FEATURE_ID_S10_SDM_MB = 0xb,
	FME_FEATURE_ID_AVMMI_BMC = 0xc,
	FME_FEATURE_ID_100G_EHIP = 0xd,
	FME_FEATURE_ID_MAX10_SPI = 0xe,
	FME_FEATURE_ID_MAC_ROM_I2C = 0xf,
	FME_FEATURE_ID_PHY_GROUP = 0x10,

	/* one for fme header. */
	FME_FEATURE_ID_MAX = 0x11,
};

/*
 * These values serve as an index into the port_features[] array,
 * and do not exactly match the hardware feature ID. The relation-
 * ship is: id = (hw_feature_id & 0xf) + 1
 */
enum port_feature_id {
	PORT_FEATURE_ID_HEADER = 0x0,
	PORT_FEATURE_ID_ERROR = 0x1,	/* HW Feature ID: 0x10 */
	PORT_FEATURE_ID_UMSG = 0x2,	/* HW Feature ID: 0x11 */
	PORT_FEATURE_ID_UINT = 0x3,	/* HW Feature ID: 0x12 */
	PORT_FEATURE_ID_STP = 0x4,	/* HW Feature ID: 0x13 */
	PORT_FEATURE_ID_IOPLL = 0x5,	/* HW Feature ID: 0x14 */
	PORT_FEATURE_ID_UAFU = 0x6,
	PORT_FEATURE_ID_MAX = 0x7,
};

/**
 * FEATURE_DRV - macro used to describe a specific feature driver
 * @n: name of this feature driver
 * @p: ops of this feature driver
 *
 * This macro is used to create a struct feature_driver that matches a
 * specific feature.
 */
#define FEATURE_DRV(n, p) \
	.name = (n), .ops = (p)
